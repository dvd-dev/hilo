...
# @pytest.fixture
# def device_fixture() -> Devices:
#    """Return the device fixture for a specific device."""
#    with (
#        patch("pyhilo.API", autospec=True) as api_mock,
#    ):
#        api_mock.log_traces = True
#        fresh_devices = json.loads(load_fixture("all_devices.json"))
#        devices = Devices(api_mock)
#        for raw_device in fresh_devices:
#            dev = devices.generate_device(raw_device)
#            if dev not in devices.devices:
#                devices.devices.append(dev)
#        return devices


# @pytest.fixture
# def mock_hilo(device_fixture: Hilo) -> Generator[MagicMock]:
#    """Return a mocked Hilo client."""
#    with (
#        patch("custom_components.hilo.Hilo", autospec=True) as hilo_mock,
#    ):
#        hilo = hilo_mock.return_value
#        hilo.devices = device_fixture
#        yield hilo
